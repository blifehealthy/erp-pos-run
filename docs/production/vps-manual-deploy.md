...
```sh
scp erp-pos-run-source.tar.gz deploy-user@vps-host:/opt/erp-pos-run/
ssh deploy-user@vps-host 'cd /opt/erp-pos-run && tar -xzf erp-pos-run-source.tar.gz'
```
...
rsync -az --delete \
  ...
  --exclude='__pycache__' \
  --exclude='*.pyc' \
  ./ deploy-user@vps-host:/opt/erp-pos-run/
```
