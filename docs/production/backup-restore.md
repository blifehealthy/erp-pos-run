...
- Uploads: `uploads_data` volume mounted at `/app/uploads`.
...
The script defaults to `docker-compose.prod.yml`, `.env.production`, and writes to `backups/erp-pos-prod-<timestamp>/`.

To write under another local directory:

```sh
./scripts/backup-production.sh /secure/backups/erp-pos
```
...
```sh
./scripts/restore-production.sh backups/erp-pos-prod-YYYYMMDDTHHMMSSZ
```
...
```sh
./scripts/restore-production.sh --yes backups/erp-pos-prod-YYYYMMDDTHHMMSSZ
```
...
```sh
COMPOSE_PROJECT_NAME=erp-pos-prod-drill ./scripts/restore-production.sh --yes backups/erp-pos-prod-YYYYMMDDTHHMMSSZ
COMPOSE_PROJECT_NAME=erp-pos-prod-drill docker compose -f docker-compose.prod.yml ps
COMPOSE_PROJECT_NAME=erp-pos-prod-drill docker compose -f docker-compose.prod.yml down -v
```
