...
Uploaded files are stored in the backend container at `/app/uploads`. In production, `docker-compose.prod.yml` mounts the `uploads_data` named volume at that path so files survive backend container rebuilds and restarts.
