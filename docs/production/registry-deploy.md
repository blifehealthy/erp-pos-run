...
```text
BACKEND_IMAGE=$IMAGE_REGISTRY/$IMAGE_NAMESPACE/erp-pos-backend:$RELEASE_VERSION
FRONTEND_IMAGE=$IMAGE_REGISTRY/$IMAGE_NAMESPACE/erp-pos-frontend:$RELEASE_VERSION
NGINX_IMAGE=$IMAGE_REGISTRY/$IMAGE_NAMESPACE/erp-pos-nginx:$RELEASE_VERSION
```
...
```sh
./scripts/rollback-production.sh --data-restore backups/erp-pos-prod-YYYYMMDDTHHMMSSZ .env.production
```
...
- Run `docker pull ghcr.io/<owner>/<repo>/erp-pos-backend:<release_version>` manually to isolate credentials or package visibility issues.
