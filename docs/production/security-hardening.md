...
- Safe non-force fix applied: `npm audit fix`
- Result after PR20: 3 remaining moderate findings in the Vite development-server chain: `esbuild <=0.24.2`, `vite <=6.4.1`, and `vite-plugin-pwa 0.20.x`
- Reason not fully fixed in PR20: npm reports the remaining fix requires `npm audit fix --force` and a breaking upgrade to `vite@8.0.16`. Production serves built static assets from nginx, not the Vite dev server, so this is accepted for go-live with operator sign-off and should be handled in a focused frontend tooling PR.

Backend audit:

- Command used for the meaningful audit: `pip-audit -r backend/requirements.txt` in a Python 3.11 container, matching the backend Dockerfile runtime
- Safe direct fixes applied across PR19 and PR20:
  - `python-multipart` from `0.0.9` to `0.0.27`
  - `fastapi` from `0.110.0` to `0.133.0`
  - explicit `starlette==1.0.1`
  - `pydantic` from `2.7.0` to `2.13.4` to satisfy the remediated FastAPI line
  - `weasyprint` from `62.3` to `68.0`
  - replaced `python-jose` with `PyJWT==2.13.0` to remove the vulnerable `pyasn1` dependency chain
  - added explicit `cryptography==48.0.0` because payment gateway token encryption uses `cryptography.fernet` directly
- Result after PR20: WeasyPrint, JWT/`pyasn1`, and Starlette findings are remediated by pinned backend dependency updates.
- Verification note: a minimal WeasyPrint render produced valid PDF bytes in the backend container. The non-root container emitted fontconfig cache warnings, so writable font cache configuration should be cleaned up in a follow-up hardening PR.
