...
PR15 added immutable local image tags. This workflow prepares immutable registry publishing so release images can be built once, stored in GitHub Container Registry, and pulled by production hosts when operators choose to deploy.
...
```text
ghcr.io/<owner>/<repo>/erp-pos-backend:<release_version>
ghcr.io/<owner>/<repo>/erp-pos-frontend:<release_version>
ghcr.io/<owner>/<repo>/erp-pos-nginx:<release_version>
```
