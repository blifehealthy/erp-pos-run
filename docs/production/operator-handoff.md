...
- [ ] Run an isolated restore drill with `COMPOSE_PROJECT_NAME=erp-pos-prod-drill`.
...
- Uploads volume: `uploads_data`, mounted in the `backend` container at `/app/uploads`.
- Redis volume: `redis_data`, mounted in the `redis` container at `/data`.
- Backups directory: `backups/erp-pos-prod-<timestamp>/` by default, or `PRODUCTION_BACKUP_ROOT` when provided.
