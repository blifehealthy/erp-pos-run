...
This directory contains documentation and templates for production readiness of the ERP-POS project.

## Intended production architecture

The ERP-POS production deployment is expected to consist of:
...
The backend stores uploaded files under `/app/uploads`, mounted from the `uploads_data` named volume. Production defaults allow JPEG, PNG, and WebP images up to 5 MB. User filenames are not used as storage names; files are written with UUID-based names after extension, MIME type, size, and path safety checks.
