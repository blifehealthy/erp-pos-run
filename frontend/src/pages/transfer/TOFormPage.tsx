              ...
              <div key={`${item.id ?? "new"}-${index}`} className="grid gap-3 rounded-xl border border-gray-200 p-4 md:grid-cols-6">
                ...
                {order?.status === "pending_approval" && canApprove ? (
                  ...
                  <div className="flex items-center text-sm text-gray-600">
                    {order?.items[index] ? `ขอ ${order.items[index].qty_requested} | อนุมัติ ${order.items[index].qty_approved ?? "-"} | ส่ง ${order.items[index].qty_sent ?? "-"} | รับ ${order.items[index].qty_received ?? "-"}` : null}
                  </div>
                )}
