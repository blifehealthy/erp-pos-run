...
export default function BranchSettingsPage(): JSX.Element {
  ...
  useEffect(() => {
    const run = async (): Promise<void> => {
      if (!settingsForm?.promptpay_target) {
        setQrPreview(null);
        return;
      }
      try {
        const payloadResponse = await fetch(`/api/v1/pos/promptpay/qr?amount=1&target=${encodeURIComponent(settingsForm.promptpay_target)}`);
        const payloadJson = await payloadResponse.json();
        const payload = payloadJson.data?.payload as string | undefined;
        if (!payload) {
          setQrPreview(null);
          return;
        }
        setQrPreview(await QRCode.toDataURL(payload));
      } catch {
        setQrPreview(null);
      }
    };
          ...
          <Card>
            <CardContent className="grid gap-4 p-5 lg:grid-cols-[1.4fr_320px]">
              <div className="space-y-4">
                <Field label="PromptPay Target">
                  <Input
                    value={settings?.promptpay_target ?? ""}
                    onChange={(event) => setSettingsForm((prev) => prev ? { ...prev, promptpay_target: event.target.value } : prev)}
                  />
                </Field>
                <p className="text-sm text-gray-500">รองรับเบอร์โทรศัพท์ 10 หลัก หรือเลขผู้เสียภาษี 13 หลัก</p>
              ...
              <div className="rounded-xl border border-gray-200 bg-gray-50 p-4">
                ...
                {qrPreview ? <img src={qrPreview} alt="PromptPay QR" className="mx-auto w-full max-w-56" /> : <div className="flex h-56 items-center justify-center rounded-lg border border-dashed border-gray-300 text-sm text-gray-500">กรอก PromptPay target เพื่อดูตัวอย่าง QR</div>}
              </div>
            </CardContent>
          </Card>
        </TabsContent>
