...
BACKUP_DIR="${BACKUP_ROOT%/}/erp-pos-prod-${TIMESTAMP}"
...
docker compose -f "$COMPOSE_FILE" run --rm --no-deps --entrypoint sh backend \
  -c 'tar -czf - -C /app/uploads .' \
...
compose_project_name=${COMPOSE_PROJECT_NAME:-erp-pos-prod}
env_file=$ENV_FILE
