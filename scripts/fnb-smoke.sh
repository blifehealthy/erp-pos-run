...
TABLE_QR_TOKEN="$(printf '%s' "$TABLE_JSON" | json_get data.qr_token)"
DINE_MENU_JSON="$(http_get "$INTERNAL_BASE_URL/api/public/menu/$TABLE_QR_TOKEN")"
...
DINE_ORDER_JSON="$(http_post_json "$INTERNAL_BASE_URL/api/public/menu/$TABLE_QR_TOKEN/orders" "{\"items\":[{\"product_id\":\"$PRODUCT_ID\",\"qty\":2,\"special_request\":\"dine smoke\"}],\"note\":\"dine-in smoke script\"}")"
DINE_SESSION_ID="$(printf '%s' "$DINE_ORDER_JSON" | json_get data.session_id)"
DINE_STATUS_JSON="$(http_get "$INTERNAL_BASE_URL/api/public/menu/$TABLE_QR_TOKEN/status?session_id=$DINE_SESSION_ID")"
...
BILL_JSON="$(http_post_json "$INTERNAL_BASE_URL/api/public/menu/$TABLE_QR_TOKEN/bill?session_id=$DINE_SESSION_ID" '{}')"
