...
VENV_DIR="${BACKEND_REGRESSION_VENV:-$TMP_BASE/erp-pos-backend-regression-venv}"
PYCACHE_DIR="${PYCACHE_DIR:-$TMP_BASE/erp-pos-backend-regression-pycache}"
...
run_with_docker() {
  ...
  DOCKER_SRC="$TMP_BASE/erp-pos-backend-regression-src"
  rm -rf "$DOCKER_SRC"
  mkdir -p "$DOCKER_SRC"
  ...
  docker run --rm \
    ...
    sh -c '
      ...
      python -m venv /tmp/erp-pos-regression-venv
      /tmp/erp-pos-regression-venv/bin/python -m pip install --upgrade pip >/dev/null
      /tmp/erp-pos-regression-venv/bin/python -m pip install -r requirements.txt >/dev/null
      export PYTHONPYCACHEPREFIX=/tmp/erp-pos-pycache
      /tmp/erp-pos-regression-venv/bin/python -m compileall -q app tests
      /tmp/erp-pos-regression-venv/bin/python -m unittest discover -s tests -p "test_*.py"
    '
  rm -rf "$DOCKER_SRC"
}
