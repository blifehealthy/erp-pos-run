...
async def get_promptpay_qr(
    ...
    payload = generate_promptpay_payload(company_target, Decimal(amount).quantize(Decimal("0.01")))
    return ok({"payload": payload, "amount": f"{Decimal(amount):.2f}", "target": company_target})
