...
class SaleService:
    ...
    async def get_sale(self, order_id: uuid.UUID, company_id: uuid.UUID) -> SaleOrder:
        order = await self.db.scalar(
            ...
            .options(
                ...
                selectinload(SaleOrder.location),
            )
        )
        if order is None:
    ...
    async def list_sales(
        ...
        if date_from is not None:
            start_at = datetime.combine(date_from, time.min, tzinfo=BANGKOK).astimezone(timezone.utc)
            filters.append(SaleOrder.created_at >= start_at)
        if date_to is not None:
            end_at = datetime.combine(date_to, time.max, tzinfo=BANGKOK).astimezone(timezone.utc)
            filters.append(SaleOrder.created_at <= end_at)

        query = select(SaleOrder)
        count_query = select(func.count(SaleOrder.id))
        if search and search.strip():
            keyword = search.strip()
            pattern = f"%{keyword}%"
            search_filter = or_(
                SaleOrder.order_number.ilike(pattern),
                SaleOrder.customer_name.ilike(pattern),
                SaleOrder.customer_phone.ilike(pattern),
                SaleOrder.customer_tax_id.ilike(pattern),
                CashierShift.shift_number.ilike(pattern),
            )
            query = query.join(CashierShift, CashierShift.id == SaleOrder.shift_id)
            count_query = count_query.join(CashierShift, CashierShift.id == SaleOrder.shift_id)
            filters.append(search_filter)

        total = await self.db.scalar(count_query.where(*filters)) or 0
        rows = await self.db.scalars(
            query
            .where(*filters)
            .options(
    ...
    async def get_receipt_data(self, order_id: uuid.UUID, company_id: uuid.UUID) -> dict[str, str]:
        ...
        return {
            "company_name": company.name if company else "ERP-POS",
